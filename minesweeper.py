...
class Sentence():
    ...
    def known_mines(self):
        ...
        """
        #literate over cells in sentences
    ...
    def known_safes(self):
        ...
        """
        # Iterate over cells in sentences
        if self.count == 0:
    ...
    def mark_mine(self, cell):
        ...
        """
        # Check if the given cell is present in the self.cells set
        if cell in self.cells:
             # If the cell is present, remove it from the self.cells set
            self.cells.remove(cell)
            # Decrement the count variable by 1 to reflect that a mine has been marked
            self.count -= 1
            return None 
...
class MinesweeperAI():
    ...
    def neighbors_cells(self, cell):
        """
        computes all neighboring cells of the board
        """
        mines = 0
        i, j = cell
        neighbors = set()
        for row in range(i-1, i+2):
            ...
            for col in range(j-1, j+2):
                # check if col is out of range of the board or cell itself
    ...
    def add_knowledge(self, cell, count):
     ...
     #1
        ...
        #3
        # get neighbors
        neighbors = self.neighbors_cells(cell)[0]
        detected_mines = self.neighbors_cells(cell)[1]
        # adjust count 
        count -= detected_mines
        # create new sentence from neighbors and count
        ...
        for sentence in self.knowledge:
            ...
            # add known safes to safes
            for know_safe in known_safes:
                self.safes.add(know_safe)
            # get list of known mines in sentence
            known_mines = sentence.known_mines()
            # add known mines to mines
            for know_mine in known_mines:
                self.mines.add(know_mine)

        #5
        # iterate over sentences in knowledge base
        ...
        for sentence2 in known_sentences:
            ...
            # if set are the same --> continue
                ...
                continue
            # check if new sentence is subset of known sentence
            if subset <= bigset:
                #get difference between subset and bigset
                diff_set = bigset - subset
                # check if there is just one extra field in KB-sentence
                if len(diff_set) == 1:
                    # check if the difference is equal to the difference in the count
                    if diff_count == 0:
                        ...
                        #add to known mine cells
                        self.mark_mine(new_mine)
                else:
                    # add new subset to knowledge base
    ...
    def make_safe_move(self):
        ...
        for move in self.safes:
            ...
            if move in self.moves_made:
                ...
                # if a safe move has been made is found --> break out of loop
                return safe_move
        return None
    ...
    def make_random_move(self):
        ...
        for pot_cell in board:
            ...
            if len(potential_move) == 0:
                ...
                # choose truly random move from all potential moves (not just the first or last potential move in the list)
